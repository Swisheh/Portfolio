...
public class Darkest extends Darkest$SUPER {
...
final int LIG = 2;
final int TOU = 1;
mash_nxt_setUpSensor(LIG, LIGHT_FLOOD);
mash_nxt_setUpSensor(TOU, TOUCH);
mash_sleep(500);
mash_nxt_motorBackward(MOT1, 90);
mash_nxt_motorBackward(MOT2, 90);
int darkest = 100;
while (! mash_nxt_isPushed(TOU))
{
int dark = mash_nxt_getLight(LIG);
if (mash_nxt_getLight(LIG) < darkest)
{
darkest = dark;
}
}
while (mash_nxt_getLight(LIG) > darkest)
{
mash_nxt_motorForward(MOT1, 90);
mash_nxt_motorForward(MOT2, 90);
}
mash_nxt_whichMotorPort(MOT1).stop();
mash_nxt_whichMotorPort(MOT2).stop();
} // end of main method
