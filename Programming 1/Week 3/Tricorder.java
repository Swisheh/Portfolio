...
public class Tricorder extends Tricorder$SUPER {
...
protected static final int SOU = 2;

protected static final int LIG = 3;

protected static final int TOU = 4;

protected static final int LAM = B;

protected static int change = 0;

protected static boolean lightOn = false;

protected static boolean lampOn = false;
...
{
mash_nxt_setUpSensor(Tricorder.TOU, TOUCH);
mash_nxt_setUpSensor(Tricorder.SOU, SOUND);
mash_nxt_setUpSensor(Tricorder.LIG, LIGHT_NOFLOOD);
mash_nxt_setUpSensor(Tricorder.PRO, PROXIMITY);
int mode = Tricorder.SOU;
int low = 9999;
int high = 0;
int value = 666;
mash_sleep(500);
mash_nxt_whichSensorPort(4).addSensorPortListener(
                new lejos.nxt.SensorPortListener() {
                   public void stateChanged(lejos.nxt.SensorPort source, 
                      int oldValue, int newValue) {
                      mash_nxt_onChange4(oldValue, newValue);
                   }
                }
             );
int dummy = mash_nxt_getDistance(Tricorder.PRO);
lejos.nxt.LCD.drawString("     SOUND     ", 1, 3);
while (true)
{
if (Tricorder.change > 0)
{
mash_sleep(200);
if (mode == Tricorder.PRO)
{
mode = Tricorder.SOU;
lejos.nxt.LCD.drawString("     SOUND     ", 1, 3);
}
else
if (mode == Tricorder.SOU)
{
mode = Tricorder.LIG;
lejos.nxt.LCD.drawString("          LIGHT", 1, 3);
}
else
{
mode = Tricorder.PRO;
lejos.nxt.LCD.drawString("PROXIMITY      ", 1, 3);
}
low = 9999;
high = 0;
change = Tricorder.change - 1;
}
else
if (Tricorder.change < 0)
{
...
if (mode == Tricorder.PRO)
{
value = mash_nxt_getDistance(Tricorder.PRO);
}
else
if (mode == Tricorder.SOU)
{
value = mash_nxt_getVolume(Tricorder.SOU);
}
else
{
value = mash_nxt_getLight(Tricorder.LIG);
}
lejos.nxt.LCD.drawString("value = " + value + "     ", 1, 4);
if (low > value)
{
low = value;
lejos.nxt.LCD.drawString("min = " + low + "        ", 1, 5);
}
if (high < value)
{
high = value;
lejos.nxt.LCD.drawString("max = " + high + "        ", 1, 6);
}
if (Tricorder.lampOn && mode == Tricorder.PRO)
{
mash_nxt_motorForward(Tricorder.LAM, java.lang.Math.max(0, 100 - value / 2));
}
else
if (Tricorder.lampOn)
{
mash_nxt_motorForward(Tricorder.LAM, value);
}
else
{
mash_nxt_whichMotorPort(Tricorder.LAM).stop();
}
}
}
} // end of main method
...
{
lightOn = ! Tricorder.lightOn;
mash_nxt_setFloodlight(Tricorder.LIG, Tricorder.lightOn);
}
}
...
protected static void mash_nxt_onChange4(int oldValue, 
               int newValue) {{
if (mash_nxt_isPushed(Tricorder.TOU))
{
lejos.nxt.Sound.beep();
lampOn = ! Tricorder.lampOn;
}
else
{
}
