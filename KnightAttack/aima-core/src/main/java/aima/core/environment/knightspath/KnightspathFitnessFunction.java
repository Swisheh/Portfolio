...
/**
 * A class whose purpose is to evaluate the fitness of NKnight individuals and to
 * provide utility methods for translating between an NKnightsBoard
 * representation and the String representation used by the GeneticAlgorithm.
 * 
 * @author Ciaran O'Reilly
 * 
 */
public class KnightspathFitnessFunction implements FitnessFunction<Integer>, GoalTest {

	private final KnightspathGoalTest goalTest = new KnightspathGoalTest();

	public KnightspathFitnessFunction() {
	...
	public double getValue(Individual<Integer> individual) {
		...
		//KnightspathBoard board = getBoardForIndividual(individual);
		//int boardSize = board.getSize();

		// Calculate the number of non-attacking pairs of knights (refer to AIMA
		// page 117).
//		List<XYLocation> kPositions = board.getKnightPositions();
//		for (int fromX = 0; fromX < (boardSize - 1); fromX++) {
//			for (int toX = fromX + 1; toX < boardSize; toX++) {
//				int fromY = kPositions.get(fromX).getYCoOrdinate();
//				boolean nonAttackingPair = true;
//				// Check right beside
//				int toY = fromY;
//				if (board.knightExistsAt(new XYLocation(toX, toY))) {
...
//		}
	...
	public KnightspathBoard getBoardForIndividual(Individual<Integer> individual) {
		int boardSize = individual.length();
		KnightspathBoard board = new KnightspathBoard(boardSize);
		for (int i = 0; i < boardSize; i++) {
			int pos = individual.getRepresentation().get(i);
			board.addKnightAt(new XYLocation(i, pos));
		}
